...
namespace XeniaManager.Installation
{
    public partial class Xenia
    {
        ...
        public async Task MousehookUpdate(JObject latestRelease)
        {
            ...
            JObject xeniaRelease = (JObject)assets.FirstOrDefault(file => file["name"].ToString() == "xenia-canary-mousehook.zip");
            ...
            Log.Information($"Download and extraction of the latest Xenia {EmulatorVersion.Mousehook} build completed.");
            ...
            Log.Information($"Xenia {EmulatorVersion.Mousehook} updated to version {ConfigurationManager.AppConfig.XeniaMousehook.Version}");
        }
    }
}
