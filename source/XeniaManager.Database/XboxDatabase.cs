...
public class XboxDatabase
{
    ...
    /// </summary>
    private static readonly string[] _databaseUrls = DatabaseUrls.XboxMarketplaceDatabase;
    ...
    /// If the primary URL fails, the system will try secondary URLs in sequence
    /// </summary>
    private static readonly string[] _gameInfoUrls = DatabaseUrls.XboxMarketplaceDatabaseGameInfo;
    ...
    public static async Task<GameDetailedInfo?> GetFullGameInfo(string titleId, CancellationToken cancellationToken = default, string? cacheDirectory = null)
    {
    ...
    public static async Task DownloadArtworkAsync(string? primaryUrl, string titleId, string? artworkFileName = null, string? savePath = null, SKEncodedImageFormat? format = null)
    {
        ...
        {
            artworkFileName = DatabaseArtworkHelper.ParseArtworkFileNameFromUrl(primaryUrl);
        ...
        if (format == null)
        {
            format = DatabaseArtworkHelper.InferImageFormatFromFileName(artworkFileName);
        ...
        async Task<bool> TryDownloadAsync(string url)
        {
            try
            {
                ...
                {
                    DatabaseArtworkHelper.ConvertToIcon(data, savePath);
                }
                else
                {
            ...
            {
                Logger.Trace<XboxDatabase>($"Download failed for {url}: {ex.Message}");
                return false;
            }
        }
        ...
        if (!string.IsNullOrEmpty(primaryUrl))
        {
            ...
            string githubPagesUrl = string.Format(DatabaseUrls.XboxMarketplaceDatabaseArtwork[0], titleId, artworkFileName);
            Logger.Trace<XboxDatabase>($"GitHub Pages URL: {githubPagesUrl}");
            if (await TryDownloadAsync(githubPagesUrl))
            {
            ...
            string rawGithubUrl = string.Format(DatabaseUrls.XboxMarketplaceDatabaseArtwork[1], titleId, artworkFileName);
            Logger.Trace<XboxDatabase>($"Raw GitHub URL: {rawGithubUrl}");
            if (await TryDownloadAsync(rawGithubUrl))
            {
