...
public class PatchesDatabase
{
    ...
    /// <param name="cacheDirectory">Optional cache directory override</param>
    /// <exception cref="AggregateException">Thrown when all database URLs fail to provide data</exception>
    public static async Task LoadNetplayAsync(CancellationToken cancellationToken = default, string? cacheDirectory = null)
        => await LoadDatabaseAsync(PatchDatabaseType.Netplay, cancellationToken, cacheDirectory);

    private static async Task LoadDatabaseAsync(PatchDatabaseType type, CancellationToken cancellationToken, string? cacheDirectory = null)
    {
    ...
    public static void AddPatchToNetplayIndex(PatchInfo patch)
