...
public class CryptoUtils
{
    ...
    {
        Logger.Debug<CryptoUtils>(
            $"Computing HMAC-SHA1 with key length: {key.Length}, inp1 length: {inp1.Length}, inp2 length: {inp2?.Length ?? 0}, inp3 length: {inp3?.Length ?? 0}, output length: {outputLen}");
    ...
    {
        Logger.Debug<CryptoUtils>(
            $"Performing RC4 operation with key length: {key.Length}, data offset: {dataOffset}, data length: {dataLen}, output length: {output.Length}");
        byte[] s = new byte[256];
        for (int i = 0; i < 256; i++)
        {
            s[i] = (byte)i;
